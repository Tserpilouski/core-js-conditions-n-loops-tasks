...
function convertToRomanNumerals(num) {
  ...
  const num2 = Math.trunc(((num / 10) % 1) * 10);
  let ans = '';
  for (let i = 0; i < num1; i += 1) {
    ans += obj[10];
  }
  return num2 === 0 ? ans : ans + obj[num2];
}
...
function getBalanceIndex(arr) {
  for (let i = 0; i < arr.length; i += 1) {
    const leftSum = calculateSum(arr, 0, i - 1);
    const rightSum = calculateSum(arr, i + 1, arr.length - 1);
